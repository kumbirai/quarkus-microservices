    ...
    public static void main(String[] args) throws IOException, InterruptedException
    {
        ...
        for (int i = 1; i <= 1000; i++)
        {
            var values = Map.ofEntries(
                    entry("title", faker.book()
                            .title()),
                    entry("author", faker.name()
                            .nameWithMiddle()),
                    entry("year", String.valueOf(2012 + new Random().nextInt(10))),
                    entry("genre", faker.book()
                            .genre())
            );
            ...
            // print status code
            LOGGER.log(Level.INFO, "statusCode: {0}", response.statusCode());
